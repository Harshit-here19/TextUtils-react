...
<div className="container">
    ...
    <div className='my-2 d-flex justify-center-center flex-wrap'>
        ...
        <button className="btn btn-primary mx-1 my-2" id="copy" onClick={handleCopyClick}>Copy</button>
        <button className="btn btn-primary mx-1 my-2" onClick={handleExtra}>Remove Extra Spaces</button>
    </div>
    </div>

    <div className="container my-3">
        <h2>Text Summary</h2>
        <p>{text.length>0 ? `${text.split(" ").length} words and ${text.length} character` : "Enter some Words or Characters"}</p>
        <p>{text.length>0 ? 0.008 * text.split(" ").length : "Enter Something above to Get Minutes"} Minutes to read</p>
